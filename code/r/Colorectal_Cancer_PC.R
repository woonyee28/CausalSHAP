...
# Load the dataset
dataset <- read_excel("C:/Users/snorl/Desktop/FYP/dataset/colorectal_cancer_predicted_probabilities.xlsx")

# Correct Group encoding

# Specify columns for X (features)
X_columns <- c('sex', 'age_interview','PC1','PC2','PC3', 'BMI', 'telomere length', 'aHEI2010score', 'aMED',	'DASH','SBP', 'DBP', 'Leisure screen time',	'smoke_never(0)_ex(1)_current(2)', 'alcohol_DailyandWeekly(1)vsMonthlyandNonDrinkers(0)', 'Prevalent_diabetes',	'z_pgs000055', 'z_pgs000734', 'Prob_Class_1')
...
nodeAttrs <- list(fontsize = setNames(rep(50, length(nodeNames)), nodeNames))
...
add_edges_based_on_ida <- function(df, threshold = 0.0) {
  for (i in 1:nrow(df)) {
    ...
    if (!is.na(effect) && (effect >= 0 || effect <= 0)) {
      g <<- add_edges(g, c(x, y))
    }
  }
}
